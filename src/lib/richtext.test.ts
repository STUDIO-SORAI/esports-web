...
describe('lexicalToHtml / relationship 節點 (延伸閱讀卡片)', () => {
  const relDoc = (value: unknown, relationTo = 'posts') =>
    lexicalToHtml(doc({ type: 'relationship', relationTo, value }))

  it('完整 post 物件：產出符合結構的「延伸閱讀」卡片，包含標題、摘要、分類、封面圖與文章連結', () => {
    const post = {
      title: '當聯盟化失去穩定',
      slug: 'franchising-crisis',
      excerpt: '探討賽事生態與商業模式重組',
      category: { name: '特戰英豪' },
      featuredImage: { url: '/media/fnatic.png' },
    }
    const html = relDoc(post)
    expect(html).toContain('class="related-post-card')
    expect(html).toContain('href="/post/franchising-crisis"')
    expect(html).toContain('延伸閱讀')
    expect(html).toContain('特戰英豪')
    expect(html).toContain('當聯盟化失去穩定')
    expect(html).toContain('探討賽事生態與商業模式重組')
    expect(html).toContain('src="/media/fnatic.png"')
    expect(html).toContain('閱讀全文')
  })

  it('沒有 featuredImage 時卡片照常產生，但不包含 img 容器', () => {
    const post = {
      title: '無圖文章',
      slug: 'no-image-post',
    }
    const html = relDoc(post)
    expect(html).toContain('href="/post/no-image-post"')
    expect(html).toContain('無圖文章')
    expect(html).not.toContain('<img')
  })

  it('featuredImage 各種 sizes 與 filename 的退讓機制', () => {
    expect(
      relDoc({
        title: 'T',
        slug: 's',
        featuredImage: { sizes: { card: { url: '/card.webp' } } },
      }),
    ).toContain('src="/card.webp"')

    expect(
      relDoc({
        title: 'T',
        slug: 's',
        featuredImage: { sizes: { hero: { url: '/hero.webp' } } },
      }),
    ).toContain('src="/hero.webp"')

    expect(
      relDoc({
        title: 'T',
        slug: 's',
        featuredImage: { filename: 'cover.jpg' },
      }),
    ).toContain('src="/api/media/file/cover.jpg"')
  })

  it('沒有 excerpt 時不產出空白 <p> 摘要標籤', () => {
    const html = relDoc({ title: 'T', slug: 's' })
    expect(html).not.toContain('<p class="text-xs')
  })

  it('標題與摘要的 HTML 特殊字元與刪除線格式會被正確處理', () => {
    const post = {
      title: '~~舊標題~~ <新戰報 & 快訊>',
      slug: 'safe-slug',
      excerpt: '包含 "引號" 與 <script>',
      category: 'A & B',
    }
    const html = relDoc(post)
    expect(html).toContain('舊標題 &lt;新戰報 &amp; 快訊&gt;')
    expect(html).not.toContain('~~')
    expect(html).toContain('&quot;引號&quot; 與 &lt;script&gt;')
    expect(html).toContain('A &amp; B')
  })

  it('未 populate 或無效的 value 安全回傳空字串', () => {
    expect(relDoc(undefined)).toBe('')
    expect(relDoc(null)).toBe('')
    expect(relDoc({})).toBe('')
    expect(relDoc(42)).toBe('')
    expect(relDoc('42')).toBe('')
    expect(relDoc({ id: 42 })).toBe('')
  })

  it('非 posts 或 briefs 的 relationTo 安全略過', () => {
    expect(relDoc({ title: '管理員', slug: 'admin' }, 'users')).toBe('')
  })

  it('relationTo 為 briefs 時標章為「延伸快訊」且連結導向資訊流錨點', () => {
    const brief = { title: '快訊標題', slug: 'brief-123' }
    const html = relDoc(brief, 'briefs')
    expect(html).toContain('href="/feed#brief-brief-123"')
    expect(html).toContain('延伸快訊')
  })
})
