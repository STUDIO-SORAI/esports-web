...
const postCache = new Map<string, PostDoc>()

/** 已經帶得出標題或 slug 的 value 不用再打 API */
const isPostPopulated = (value: unknown): boolean => {
  if (!value || typeof value !== 'object') return false
  const v = value as PostDoc
  return Boolean(v.title || v.slug)
}

/** 從 relationship 節點的 value 取出文章 ID；已 populate 或無效時回傳 null */
export const relationshipValueToId = (value: unknown): number | string | null => {
  if (typeof value === 'number') return value
  if (typeof value === 'string') return /^\d+$/.test(value) ? value : null
  if (value && typeof value === 'object' && !isPostPopulated(value)) {
    const id = (value as PostDoc).id
    if (typeof id === 'number' || typeof id === 'string') return id
  }
  return null
}

/** 掃出整份內容裡還沒 populate 的 relationship（文章關聯）節點 ID（已去重） */
export function collectUnpopulatedRelationshipIds(content: unknown): (number | string)[] {
  const root = (content as { root?: UnknownNode } | null | undefined)?.root
  const ids = new Set<number | string>()
  walk(root, (node) => {
    if (node.type !== 'relationship') return
    if (node.relationTo && node.relationTo !== 'posts') return
    const id = relationshipValueToId(node.value)
    if (id !== null) ids.add(id)
  })
  return [...ids]
}
...
/**
 * 用查到的文章物件回填 relationship 節點。回傳新物件，不動到傳進來的內容。
 */
export function populateRelationships(
  content: unknown,
  postsById: Map<string, PostDoc>,
): unknown {
  if (!content || typeof content !== 'object') return content
  const clone = structuredClone(content) as { root?: UnknownNode }
  walk(clone.root, (node) => {
    ...
    if (post) node.value = post
  })
  return clone
}

/** 逐一向 CMS 取文章文件（以 depth=1 帶出封面圖與分類），結果會快取 */
export async function fetchPostsByIds(
  ids: (number | string)[],
  fetchImpl: typeof fetch = fetch,
): Promise<Map<string, PostDoc>> {
  const result = new Map<string, PostDoc>()
  await Promise.all(
    ids.map(async (id) => {
      const key = String(id)
      ...
      try {
        const res = await fetchImpl(assetUrl(`/api/posts/${encodeURIComponent(key)}?depth=1&draft=true`))
        if (!res.ok) return
        const doc = (await res.json()) as PostDoc
        if (doc && (doc.title || doc.slug)) {
          postCache.set(key, doc)
          result.set(key, doc)
        }
      } catch {
        // 取不到就維持原狀，不要讓整篇預覽掛掉
