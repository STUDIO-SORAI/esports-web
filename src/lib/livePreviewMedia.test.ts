...
describe('relationshipValueToId', () => {
  it('數字與純數字字串是文章 ID', () => {
    expect(relationshipValueToId(12)).toBe(12)
    expect(relationshipValueToId('12')).toBe('12')
  })

  it('已經帶有 title 或 slug 的物件不用再補', () => {
    expect(relationshipValueToId({ id: 7, title: '文章' })).toBeNull()
    expect(relationshipValueToId({ id: 7, slug: 'post-slug' })).toBeNull()
  })

  it('只有 id 的物件要補', () => {
    expect(relationshipValueToId({ id: 7 })).toBe(7)
  })

  it('沒有 value 就沒有 ID', () => {
    expect(relationshipValueToId(undefined)).toBeNull()
    expect(relationshipValueToId({})).toBeNull()
  })
})

describe('collectUnpopulatedRelationshipIds', () => {
  it('挖出文章關聯節點中的 ID 並去重', () => {
    const content = doc(
      { type: 'relationship', relationTo: 'posts', value: 12 },
      {
        type: 'paragraph',
        children: [
          { type: 'relationship', relationTo: 'posts', value: { id: 12 } },
          { type: 'relationship', relationTo: 'posts', value: 34 },
        ],
      },
      { type: 'relationship', relationTo: 'posts', value: { id: 56, title: '已帶標題' } },
      { type: 'relationship', relationTo: 'users', value: 99 },
    )
    expect(collectUnpopulatedRelationshipIds(content)).toEqual([12, 34])
  })
})

describe('populateRelationships', () => {
  it('回填之後 lexicalToHtml 才產得出延伸閱讀卡片', () => {
    const content = doc({ type: 'relationship', relationTo: 'posts', value: 12 })
    expect(lexicalToHtml(content)).toBe('')

    const filled = populateRelationships(
      content,
      new Map([['12', { title: '回填標題', slug: 'filled-slug' }]]),
    )
    expect(lexicalToHtml(filled)).toContain('class="related-post-card')
    expect(lexicalToHtml(filled)).toContain('回填標題')
  })

  it('不改動傳進來的原始物件', () => {
    const content = doc({ type: 'relationship', relationTo: 'posts', value: 12 })
    populateRelationships(content, new Map([['12', { title: 'T', slug: 's' }]]))
    expect((content.root.children[0] as any).value).toBe(12)
  })
})

describe('fetchPostsByIds', () => {
  it('打 posts API 並回傳 id -> 文章物件', async () => {
    const fetchImpl = vi.fn(async () =>
      new Response(JSON.stringify({ id: 12, title: '文章', slug: 'p12' }), { status: 200 }),
    )
    const map = await fetchPostsByIds([12], fetchImpl as any)
    expect(fetchImpl.mock.calls[0][0]).toContain('/api/posts/12')
    expect(map.get('12')?.title).toBe('文章')
  })

  it('API 掛掉或 404 時安靜跳過', async () => {
    const boom = vi.fn(async () => {
      throw new Error('fail')
    })
    await expect(fetchPostsByIds([999], boom as any)).resolves.toEqual(new Map())
  })
})
