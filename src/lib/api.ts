...
function mapPayloadDocToPost(doc: any): Post {
  ...
  const parseNotebookImages = (imageUrls?: string, content?: string) => {
    const images: { url: string; caption?: string }[] = [];
    if (imageUrls && typeof imageUrls === 'string') {
      const lines = imageUrls.split('\n').map((l) => l.trim()).filter(Boolean);
      for (const line of lines) {
        const [rawUrl, ...capParts] = line.split('|');
        const u = rawUrl.trim();
        const cap = capParts.join('|').trim();
        if (u) {
          images.push({ url: assetUrl(u), caption: cap || undefined });
        }
      }
    }
    if (images.length === 0 && content && typeof content === 'string') {
      const mdImgRegex = /!\[(.*?)\]\((https?:\/\/[^\s\)]+|\/[^\s\)]+)\)/g;
      let match: RegExpExecArray | null;
      while ((match = mdImgRegex.exec(content)) !== null) {
        images.push({ url: assetUrl(match[2]), caption: match[1] || undefined });
      }
    }
    return images;
  };

  const rawNotebooks = Array.isArray(doc.notebooks) ? doc.notebooks : [];
  const notebooks = rawNotebooks
    .map((n: any) => {
      const title = n.title || '';
      const content = n.content || '';
      const imageUrls = n.imageUrls || '';
      const images = parseNotebookImages(imageUrls, content);
      return {
        id: n.id ? String(n.id) : undefined,
        title,
        source: n.source || '',
        placement: n.placement || 'default',
    ...
    .filter((n: any) => n.title && (n.content || (n.images && n.images.length > 0)));
 ...
 *
 * 影片與動態 GIF 一律吃原檔：sharp 產出的派生檔是靜態單張，
 * 拿 sizes.full 會把動畫壓成一張定格。
 */
function pickUploadImage(upload: any): UploadImage | null {
  if (!upload || typeof upload !== "object") return null;
  const mimeType = String(upload.mimeType || "");
  const animated = mimeType.startsWith("video/") || mimeType === "image/gif";
...
export function getReadTime(post: Post | string): string {
  const content =
    typeof post === "string"
      ? post
      : post.rawContent || post.body || post.content || "";
  if (content) {
    const cleanText = content.replace(/<\/?[^>]+(>|$)/g, "").replace(/[#*`_\[\]()]/g, "").trim();
    const time = Math.ceil(cleanText.length / 600);
    return `${Math.max(1, time)} 分鐘`;
  }
  const words = typeof post === "object" ? Number(post.wordCount || 0) : 0;
